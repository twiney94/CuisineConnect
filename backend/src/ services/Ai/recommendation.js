...
const recipeRecommendation = async (req, res) => {
  ...
  try {
    ...
    const recipes = openaiResponse.recipes;
...
function generatePromptSystem(language) {
  return (
    "you are a chef and all your answers will be formatted as JSON key/value" +
    "Answer in this " +
...
function generatePrompt(dish) {
  return (
    "Give me similar recipes to make the following dish " +
    JSON.stringify(dish) +
    "return as an array recipes and respect the name of the array recipes"
  );
}

function generateAccompanimentPrompt(dish) {
  return (
    "offer accompaniments for this" +
    JSON.stringify(dish) +
    "such as wine, desserts or cheeses " +
